...
{//convert from rads to deg 
		...
		//returns remainder of rads / PI 
	{
		...
		if (result < 0) result += PI; return result;
	}
	...
	{ //if value less than min, set as max 
		...
		//alterantively, if greater than max, set as min 
		else if (value > max) value = min; return value;
	}
	//Limit value according to range 
	double Math::Limit(double value, double min, double max)
	{ //if value less than min, set as min 
		if (value < min) value = min; //alterantively, if greater than max, set as max 
		else if (value > max) value = max; return value;
	}
	...
	// Calculate V direction based on degree angle
	...
	{ //get X and V directions from angle 
		double vx = DirectionX(angle);
		double vy = DirectionX(angle);
		//send vector for C/V direction 
		return Vector2(vx, vy);
	}

	//compare values and get angle to target (in radians) 
	double Math::AngleToTarget(double x1, double y1,
		double x2, double y2)
	{
	...
	{
	//use exiting method to calculate 
...
//can also be used for getting angle between two vectors 
	...
	double Math::DotProduct(double xl, double y1, double z1, double x2, double y2, double z2)
	{ //adds the multiplication of 
	  //relative dimensions between 
	  //both vector values 

		return(xl*x2 + y1*y2 + z1*z2);
	}

	double Math::DotProduct(double xl, double yl, 
							double x2, double y2)
	{
		//adds the multiplication of 
		//relative dimensions between 
		...
		double Math::DotProduct(Vector3& A, Vector3& B) {
			...
			//uses existing method to perform calculation 
	...
	//when given forward vector and up vector 
									...
									double x2, double y2, double z2)
		{
			...
			Vector3 Math::CrossProduct(Vector3& A, Vector3& B) {
				//use existing method to claculate 
				return CrossProduct(A.getX(), A.getY(), A.getZ(),
					B.getX(), B.getY(), B.getZ());
			}

			//Normalising vectors gives them a magnitude of 1 (Unit vectors) 
