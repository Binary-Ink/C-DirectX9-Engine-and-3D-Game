...
bool Skybox::Create(std::string sharedFilename) {
	...
	for (int i = 0; i < 4; i++) {
		...
		g_engine->getDevice()->SetSamplerState(i, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
		g_engine->getDevice()->SetSamplerState(i, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
		g_engine->getDevice()->SetSamplerState(i, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
	}
	...
	for (int n = 0; n < 6; n++) {
		if (!textures[n]) textures[n] = new Texture();
	}
...
void Skybox::Render() {
	...
	g_engine->getDevice()->SetSamplerState(0, D3DSAMP_ADDRESSW, D3DTADDRESS_WRAP);
}