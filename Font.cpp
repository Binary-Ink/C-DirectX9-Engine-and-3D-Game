...
namespace Batman
{
	...
	void Font::Print(int x, int y, string text, D3DCOLOR color)
	{
		...
		*/
		RECT rect = { x, y, 0, 0 };
		fontObj->DrawTextA(NULL, text.c_str(), text.length(),
			&rect, DT_CALCRECT, color);

		//print the text
		fontObj->DrawTextA(g_engine->getSpriteObj(), text.c_str(),
	...
	int Font::getTextWidth(std::string text)
	{
		...
		RECT rect = { 0,0,0,0 };
		fontObj->DrawTextA(NULL, text.c_str(), text.length(), &rect,
