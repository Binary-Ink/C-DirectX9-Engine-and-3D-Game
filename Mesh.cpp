...
//destructor - release memory (inc. textures) 
Mesh::~Mesh(void) { if (materials != NULL) delete[] materials; 
	if (textures != NULL) {
...
void Mesh::createCylinder(float radiusl, float radius2, float length, int slices, int stacks) {
D3DXCreateCylinder(g_engine->getDevice(), radiusl, radius2, length, slices, stacks, &mesh, NULL); }
...
void Mesh::splitPath(string& inputPath, string* pathOnly, string* filenameOnly) {
 ...
 //Render methods 
 void Mesh::Render() { Render(p_effect,"Texture"); } 
 ...
 void Mesh::Render(Effect *effect, std::string fxTextureParam) {
	 ...
	 if(material_count==0) { mesh->DrawSubset(0); }
	 else
	 {
		 //otherwsie we loop through //draw each mesh subset 
			 for(DWORD i=0; i < material_count; i++) {
		 //set the texture used by this face 
				 if(textures[i]) { p_effect->setParam(fxTextureParam, (textures[i])); }
...
rotation.x += (float)x; rotation.y += (float)y; rotation.z += (float)z;
}
//set rotate values 
...
BSPHERE Mesh::getBoundingSphere()
{
	...
	//center and radius of bounding sphere
	D3DXVECTOR3 center;
	float radius;
	...
	//NB: This could be adjusted for scale
...
{
	//Rather than creating a fixed array size and setting to read only
	//on the vertex buffer, the memory can be more easily allocated
	//once this method is completed

	//(Here we're creating a buffer to store vertices of the mesh)
	BYTE* pVertices = 0;
