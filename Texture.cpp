...
//default constrcutor - NULLS object pointers 
...
bool Texture::Load(std::string filename, Color transcolor) {
	...
	//create the new txture by loading a bitmap image file 
	D3DXCreateTextureFromFileEx(
		g_engine->getDevice(), //D3D device object
		filename.c_str(), 	//bitmap filename
		info.Width, //bitmap image width
		info.Height, //bitmap image
		...
		&info,//bitmap file info (from file) 
		NULL,//color palette  
...
bool Texture::createRenderTarget(int width, int height) {
	...
	g_engine->getDevice()->CreateTexture(width, //width 
		height, //height 
		1, //mip levels 
	...
	g_engine->getDevice()->CreateDepthStencilSurface(width, //width 
		height, //height 
		D3DFMT_D16, //format 
...
// Used only when texture is a render target and never when the primary device is rendering! 
...
bool Texture::renderStart(bool clear, bool sprite, Color clearColor) 
{
	...
	if(clear) g_engine->getDevice()->Clear(0, NULL, D3DCLEAR_TARGET, clearColor.ToD3DCOLOR(), 1.0f, 0); 
...
// Saves a texture in memory to a file. Supports TGA, )PG, PNG, DDS, BMP (default).
