...
namespace Batman
{
	...
	BoneMesh::~BoneMesh(void)
	{
		...
		if (p_frameRoot)
		{
			...
			//remove all frames 
			D3DXFrameDestroy(p_frameRoot, &memoryAllocator); p_frameRoot = 0;
		}
	...
	void BoneMesh::createBoneMatrices(D3DXFRAME_NEW *pFrame, LPD3DXMATRIX pParentMatrix)
	{
		...
		if (pMesh) {
			...
			if (!p_firstMesh) p_firstMesh = pMesh;
		...
		if (pFrame->pFrameSibling) createBoneMatrices((D3DXFRAME_NEW*)pFrame->pFrameSibling, pParentMatrix);
	...
	void BoneMesh::updateFrameMatrices(D3DXFRAME *frameBase, D3DXMATRIX *parentMatrix)
	{
		D3DXFRAME_NEW *currentFrame = (D3DXFRAME_NEW*)frameBase;
		//if parent matrix exists multiply with new frame matrix 
		if (parentMatrix != NULL) D3DXMatrixMultiply(&currentFrame->combinedMatrix,
		...
		if (currentFrame->pFrameFirstChild != NULL) {
			updateFrameMatrices(currentFrame->pFrameFirstChild, &currentFrame->combinedMatrix);
		}
	}

	void BoneMesh::Render(Batman::Effect* effect) {
		if (p_frameRoot) drawFrame(p_frameRoot, effect);
	}
	...
	void BoneMesh::buildBoneList(LPD3DXFRAME frame, vector<std::string> &names) {
		if (frame->Name) names.push_back((std::string) frame->Name);
		if (frame->pFrameSibling != NULL) buildBoneList(frame->pFrameSibling, names);
		if (frame->pFrameFirstChild != NULL) buildBoneList(frame->pFrameFirstChild, names);
	}
	...
	void BoneMesh::drawFrame(LPD3DXFRAME frame, Batman::Effect* effect) {
		...
		if (frame->pFrameSibling != NULL) drawFrame(frame->pFrameSibling, effect);

		//recursively draw children 
		if (frame->pFrameFirstChild != NULL) drawFrame(frame->pFrameFirstChild, effect);
	}
		...
		LPD3DXFRAME frameBase, Batman::Effect* effect)
	{
		...
		for (unsigned int i = 0; i < meshContainer->NumMaterials; i++) {
			...
			effect->Begin(); mesh->DrawSubset(i); effect->End();
		}
	}
	...
	void BoneMesh::setAnimationSet(unsigned int index) {
		...
		p_animController->KeyTrackEnable(p_currentTrack, FALSE, p_currentTime + TransitionTime);

		//change the speed right away so the animation completes 
		p_animController->KeyTrackSpeed(p_currentTrack, 0.0f, p_currentTime, TransitionTime,
			D3DXTRANSITION_LINEAR);

		//change the weighting (blending) of the current track 
		p_animController->KeyTrackWeight(p_currentTrack, 0.0f, p_currentTime, TransitionTime,
			D3DXTRANSITION_LINEAR);
		...
		p_animController->KeyTrackSpeed(newTrack, 1.0f, p_currentTime, TransitionTime,
			D3DXTRANSITION_LINEAR);

		//change the weighting of the current track (0.0 to 1.0) 
		p_animController->KeyTrackWeight(newTrack, 1.0f, p_currentTime, TransitionTime,
			D3DXTRANSITION_LINEAR);
		//save current track 
	...
	void BoneMesh::nextAnimation() {
		...
		if (newAnimationSet >= p_numAnimationSets) newAnimationSet = 0;
		setAnimationSet(newAnimationSet);
	}

	std::string BoneMesh::getAnimationSetName(unsigned int index) {
		if (index >= p_numAnimationSets) return "Error: No set exists";
	...
	LPD3DXMESHCONTAINER *retNewMeshContainer)
{
	...
	*retNewMeshContainer = 0;
	...
	newMeshContainer->MeshData.pMesh = meshData->pMesh; newMeshContainer->MeshData.pMesh->AddRef();
	...
	ZeroMemory(newMeshContainer->textures, sizeof(LPDIRECT3DTEXTURE9) * newMeshContainer->NumMaterials);
	...
	if (numMaterials > 0)
	{
		...
		for (DWORD i = 0; i < numMaterials; ++i)
		{
			...
			if (pSkinInfo) {
				...
				// BNo skin info so 0 all the pointers 
...
HRESULT MeshLoaderCallback::DestroyMeshContainer(LPD3DXMESHCONTAINER meshContainerBase) {
	...
	//delete texture array
		if (meshContainer->textures) delete []meshContainer->textures; 
	...
	//delete bone parts
		if (meshContainer->boneOffsets) { delete []meshContainer->boneOffsets; 
...
void MeshLoaderCallback::splitPath(string& inputPath, string* pathOnly, string* filenameOnly)
{
	...
	if (lastSlashPos == string::npos) { *pathOnly = ""; *filenameOnly = fullPath; }
...
bool MeshLoaderCallback::findFile(string *filename) {
	...
	if (doesFileExist(filenameOnly)) {
		*filename = filenameOnly; return true;
	}
	//not found 
	return false;
}
