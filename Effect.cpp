...
namespace Batman
{
	...
	void Effect::setParam(std::string name, Vector3 vector) {
		//set a fourth value before setting vector 
		D3DXVECTOR4 v; v.x = (float)vector.x;
		v.y = (float)vector.y;
		v.z = (float)vector.z;
		v.w = 0; p_effect->SetVector(name.c_str(), &v);
	}
